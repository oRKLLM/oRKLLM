/* rknpu_sched_i8.c — int8/w8a8 single-submit M-scheduler (int8 twin of rknpu_sched.c).
 * One RKNPU_SUBMIT; the NPU iterates internal M-tiles. DECODE (M=1) does ANY K in one
 * submit (0x1010=0x20, 0x1040=0xb1 — same as fp16, captured from librknnrt int8).
 * int8 reg differences vs fp16: 0x1030=K*N, 0x1034=K, 0x1044=ceil(K/64), 0x107c=K/16,
 * weights [Ntile][Ktile][32][32], R=65536/K, 0x1040 multi-tile uses effective K=K/2.
 *   cc -O2 -I. -o rknpu_sched_i8 rknpu_sched_i8.c && sudo ./rknpu_sched_i8 [M K N r1010 r1040]
 * Constraints: K%32==0, N%32==0. */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
int main(int argc,char**argv){
    int M=argc>1?atoi(argv[1]):1,K=argc>2?atoi(argv[2]):4096,N=argc>3?atoi(argv[3]):32;
    ...
    struct buf A=bcreate((size_t)M*K,0x403),B=bcreate((size_t)K*N,0x403),C=bcreate((size_t)M*N*4,0x403);
    int8_t*a=A.cpu,*bbuf=B.cpu; int32_t*href=malloc((size_t)M*N*4);
    int8_t*alog=malloc((size_t)M*K),*blog=malloc((size_t)K*N); unsigned s=12345;
    ...
    setr(rc,REGCMD_I8_N,0x201,0x1020,0x10000|M);setr(rc,REGCMD_I8_N,0x201,0x1084,0x10000|M);setr(rc,REGCMD_I8_N,0x201,0x102c,M);
    ...
    /* SINGLE M-tile (M<R, incl decode M=1): full K in one submit, 0x1040=0xb1.
     * MULTI-tile K<=1024: int8 closed-form at effective K=K/2. K>1024 multi-tile:
     * nonlinear CBUF heuristic — pass explicit hex args. */
    int R=65536/K; if(R<1)R=1;
    if(!r1010){ int rows=(M+1<R)?(M+1):R; r1010=16*rows; }
    if(!r1040){ if(M<R){ r1040=0xb1; }
                else { int keff=K/2,kk=keff/256,lg=0; while(kk>1){kk>>=1;lg++;}
                int base=0xb1-15*((1<<lg)-1), slope=15*(1<<lg), mg=M/64; if(mg<1)mg=1;
                int v=base-slope*(mg-1); if(v<0x1b)v=0x1b; r1040=v; } }
    setr(rc,REGCMD_I8_N,0x201,0x1010,r1010);
    setr(rc,REGCMD_I8_N,0x201,0x1040,r1040);
    ...
    int32_t*c=C.cpu;int bad=0;for(int i=0;i<M*N;i++)if(c[i]!=href[i])bad++;
    printf("SCHED-I8 MKN=%d,%d,%d 1010=0x%x 1040=0x%x  ONE submit  mism=%d/%d : %s  hw=%lldus\n",
