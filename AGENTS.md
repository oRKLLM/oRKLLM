...
| `src/config.js` (chipset) | `getPlatform()` (SoC slug from `/proc/device-tree/compatible`) + `getNpuCoreCount()` (rk3576→2, rk3588→3, else 1) — single source of truth, surfaced in `/api/admin/status` and `/api/admin/global-settings` `server` as `platform`/`npuCores` (the Settings NPU-pool-size slider caps its max at `npuCores`) |
