...
| `frontend/src/views/Models.vue` | Model manager + HF search/collection browser/downloader; recursive model scan; platform-aware search; download queue grouped by repo. Manager tab uses `GET /api/admin/library` to show three categories — Available Models (`.rkllm`), Base Models, and Eagle-3 Draft Heads (with an "Add embeddings" dialog: downloaded base model or base-repo slice → `POST /api/admin/eagle3/embeddings`). Wide-screen responsive (`.page-container` 1400px): main-list + sidebar on lg+ |
