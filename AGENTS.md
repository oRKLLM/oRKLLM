...
| `src/addon/orkllm_llama_napi.cpp` | C++ N-API addon for the llama/GGUF backend; `dlopen`s `libllama.so`; same method surface as `orkllm_napi.cpp` (`load_library`, `init_model`, `run`, `abort_inference`, `clear_kv_cache`); uses `llama_state_seq_save_file`/`llama_state_seq_load_file` for prefix-cache integration; KV cache file extension `.llamacache`. Sampling/generation params are applied **per-run** (the `run` IPC message carries `options`; the worker forwards `max_new_tokens`/`temperature`/`top_p`/`top_k`): `run` reads `max_new_tokens` for the generation cap and rebuilds the full sampler chain each call from the request's params, so a model's settings take effect without a reload. The chain is `[penalties?]→top_k→top_p→[min_p?]→temp→dist`, or `[penalties?]→temp→mirostat_v2` when mirostat>0; `repeat_penalty`/`presence_penalty`/`frequency_penalty` (via `llama_sampler_init_penalties`), `mirostat`/`mirostat_tau`/`mirostat_eta` (via `_mirostat_v2`), and `min_p` are all `dlsym`'d optionally (nullptr-guarded — skipped if libllama lacks them). The `dist`/`mirostat` seed is `LLAMA_RANDOM_SEED` (0xFFFFFFFF) so output varies per run. `ctx_window`→`n_ctx` and `thinking_enabled` (prompt seed in `routes.js`) also apply. Penalties/mirostat come from the saved per-model settings regardless of `force_sampling` (they aren't in the OpenAI request body). **Embeddings mode** (init-time, `llama_context_params.embeddings`): opt-in via the `embeddings` load option, default **false**. It was unconditionally true, which made the graph emit output for every position rather than the one the batch asks for, so `lm_head` ran once per prompt token — measured on RK3588 (Qwen3-1.7B, same model/pack/process, ordering-controlled) at **39.1 tok/s prefill on vs 133.2 off, a 3.4x tax on every chat completion**. `n_outputs_max` follows it (1 for generation, `n_batch` for embeddings). Only `infer_mode: 1` (last hidden layer, Eagle-3) needs it — `/v1/embeddings` is a mock and never reaches the addon — so `pool.js` sets it from the per-model `speculative_mode === 'eagle3'` and includes it in the slot-reuse key. A context built without it returns null from `llama_get_embeddings_ith`, which the Eagle path already handles as "hidden states unavailable" and falls back to standard generation. **KV-cache quantization** (init-time): `init_model` reads `kv_type_k`/`kv_type_v` from the load options and sets `llama_context_params.type_k`/`type_v` via `kvTypeFromStr` (f16=1, q4_0=2, q5_1=7, q8_0=8, **turbo2/3/4 = 42/43/44** — TurboQuant WHT+polar KV compression); turbo/quantized types require flash attention which the runtime auto-enables, so `flash_attn_type` is left at AUTO. Defaults to f16/f16; changing it forces a model re-init (resolved per-model in `pool.js`). Requires a runtime build that includes TurboQuant; runs the WHT/quant on the Mali GPU via the Vulkan backend. **Debug switches** (all off by default, product behaviour byte-for-byte unchanged when unset): `ORKLLM_LOGIT_DEBUG=1` dumps the resolved `llama_context_params` (`[CPAR-DBG]`), every decode batch's pos/seq/logits flags (`[BATCH-DBG]`), and the top-5 logits + full prompt token ids at end of prefill (`[LOGIT-DBG]`); `ORKLLM_RTLD_GLOBAL=1` dlopens libllama with `RTLD_GLOBAL` instead of `RTLD_LOCAL`, which changes what ggml's own dlopen'd backends bind against; `ORKLLM_NO_FADVISE=1` suppresses the post-load `posix_fadvise(DONTNEED)` on the model file — that call is safe for a normal GGUF (dropped pages re-fault from the file) but a pack-backed load is handed a sparse STUB whose packed tensors are file holes, so anything re-faulted there reads as zeros. The `selftest(prompt)` export decodes one prompt via `llama_batch_get_one` and prints the same top-5 — it mirrors a CLI frontend's call shape, which is how the b10687-ork stub-fusion bug was isolated (a frontend requests logits for the last token only, so the final layer runs at M=1 and never fuses; this addon leaves all prompt rows live, so it does). **Unified control + single-pass SSD blob**: the per-model "KV Cache Compression" dropdown (`kv_cache_quant`) is the single UI control — its TurboQuant levels (`turbo2/3/4`, shown only for llama models) are mapped in `pool.js` to `kv_type_v` (+ `kv_type_k=q8_0`, the asymmetric "never turbo K" policy). Because `llama_state_seq_save_file` serializes the KV cache in its native `type_v`, a turbo V-cache is written to the prefix-cache `.llamacache` blob already turbo-compressed — the same single GPU pass that built the live cache. The llama prefix cache is therefore a **separate implementation** from the rkllm one (`cache.js` `putCachePath` dispatches on `runtime`): the llama path never runs the `kvcache_quant_napi` PolarQuant (which understands only the rkllm `.rkllmcache` layout and, for turbo, would be a redundant lossy second pass). The legacy SSD PolarQuant (q8/pq8/pq4) is rkllm-only. **Selective Vulkan mode + n_gpu_layers** (init-time): `init_model` reads `vk_mode` (`turboquant`→1 / `prefill`→2 / else 0) and, if the optional `ggml_vk_set_mode` symbol is present in the runtime (nullptr-guarded), calls it **before model load** to scope the Vulkan backend; `n_gpu_layers` is also a load option (default 999). `pool.js` derives these from turbo: with turbo KV it sets `vk_mode:'turboquant'` + `n_gpu_layers:0` (Vulkan does only the KV/turbo ops, model weights/layers stay on the NPU — avoids the recurrent multi-turn decode corruption that all-on-Vulkan causes, and the per-decode VRAM↔RAM copy); without turbo the worker disables Vulkan entirely (`GGML_DISABLE_VULKAN`) and layers run on the NPU. **`use_mmap`** (init-time, `llama_model_params.use_mmap`): a load option, defaulted **true for the gguf path by `pool.js`** — file-backed mmap is fully reclaimable under memory pressure and is necessary to load models >15B on a 32 GB board without OOM (previously defaulted false due to virtual size mapping misconceptions). Overridable per-model via a `use_mmap` setting |
| | `src/addon/orkllm_llama_napi.cpp` (prefill chunking) | Prefill is issued in chunks of at most `n_batch` (512), and the division is **not** neutral. Measured per-token cost on RK3588 (Qwen3-1.7B int4 `.orkpack`, warm): `M=32` 7.27, `M=64` 4.78, `M=128` 3.79, **`M=256` 3.63**, `M=384` 3.85, `M=512` 4.08 ms/token — so a chunk below ~128 pays a large fixed per-call cost amortised over almost nothing (`ORK_MINM` — default **8** in ggml-ork — is where the NPU MUL_MAT route becomes *available*, not where the chunk size becomes optimal; a direct CPU-vs-NPU sweep on 2026-09-04 found the NPU already 2.08x faster at M=8 and 14.5x at M=512, i.e. no crossover above 8, so availability is not the constraint — chunk shape is). **`ORK_MINM` does not apply to a pack-backed model at all:** when the loaded source is a stub, ggml-ork sets `source_is_stub` and `supports_op` then claims *every* pack-owned matmul unconditionally — including M=1 — because declining would hand ggml a file hole and it would compute from zeros (measured upstream on Qwen3.6-27B: the stub source produced garbage output that perplexity did not catch, since PPL is teacher-forced over the M>1 prefill path). Removing an `ORK_MINM=32` override from a board serving an `.orkpack` measured 58.97 -> 59.23 tok/s on a 23-token prompt — no change, because the gate was never live. The corollary is that **decode on a pack-backed model is NOT routed to the CPU** by the M=1 decline, so the 5.40-vs-12.16 tok/s blocking-path penalty may be live in serving; unverified, worth checking directly, and the 512 cap is itself ~12% off the optimum. `ork_plan_prefill_chunks(n, cap, floor, strategy)` implements three strategies, selected by `ORKLLM_CHUNK_STRATEGY` (`fill` default / `balanced` / `borrow`) with `ORKLLM_CHUNK_FLOOR` (default 128): **fill** is current behaviour and leaves `n % cap` as the tail (519 → 512+7, and that tail measured 63.5 ms/token); **balanced** uses `k=ceil(n/cap), M=ceil(n/k)` so `M` lands in `[cap/2, cap]` and no tail is ever tiny (519 → 260+259); **borrow** keeps chunks at the cap but lifts a sub-floor tail to exactly the floor by taking from the donor, and declines if that would push the donor below the floor (519 → 391+128). `n <= cap` is always ONE chunk under every strategy — a 15-token prompt must not become 3×5. **MEASURED** on RK3588 / b10701-ork / Qwen3-1.7B-UD-Q4_K_XL.orkpack (5 reps per length, settled = mean of the last 3; pairs whose plans are identical put the noise floor at 1-2% above the cap): geomean **fill 73.84 / balanced 75.24 / borrow 76.41 tok/s**. Per length — 402 (control, one chunk in all three) 87.4/85.4/90.0; **520** 72.8 `[512 8]` / 77.2 `[260 260]` / **81.8** `[392 128]`; **529** 74.7 / **79.8** / 78.5; **639** 74.5 / **76.8** / 75.7; 696 72.6/73.0/71.3 and 1021 63.0/61.5/63.7 (borrow's plan is identical to fill at both, tail already >= floor). The headline effect: a 402-token prompt prefills at 87 tok/s and adding 118 tokens to make 520 DROPS it to 73, because the division becomes `[512, 8]`. The recovered penalty shrinks as the tail grows (tail 8 -> +6%, 17 -> +7%, 127 -> +3%, 184 -> nothing), which is why a floor is the right shape of fix. **`borrow` is the default** — beyond the higher geomean it is a LOCAL repair (chunks stay at the cap, only a sub-floor tail is rewritten, so most plans are byte-identical to fill, where balanced makes M a function of n on every request), and the benchmark UNDERSTATES that: replaying six fixed lengths lets balanced's derived M warm up by rep 2, while real traffic varies prompt length continuously and NPU prefill warm-up is shape-dependent (`ork_bench`) — so the measurement is biased toward balanced and borrow still wins. All three stay selectable because the result is chipset-, model- and tier-specific. **Floor swept** on the same board/model under borrow: geomean **128 -> 76.33, 192 -> 77.76, 256 -> 77.77 tok/s** (per length 520 82.2/81.7/79.9, 529 78.3/80.6/81.8, 639 74.0/76.2/76.4, 696 71.3/72.9/73.3). 192 and 256 tie and both beat 128 by ~1.9%, so **`ORKLLM_CHUNK_FLOOR` defaults to 192** — chosen over 256 on a structural argument the geomean cannot see: at 256 a two-chunk prompt degenerates INTO balanced (`n=520` plans as `[264 256]`, an even split sized from `n`), reintroducing the novel-shape cost that made borrow preferable, and 520 is exactly where 256 measured worst. Net effect of the new defaults vs the shipped `fill`/128 behaviour on these four lengths: **73.66 -> 77.76 tok/s, +5.6%**. This is the ECONOMIC floor (below which a call cannot amortise its fixed cost), not `ORK_MINM` (default 8; merely where the NPU MUL_MAT route becomes available). `scripts/bench-prefill-chunking.mjs` measures all three; `plan_prefill_chunks` is exported for unit testing (`test/prefill_chunking.test.mjs`) |
...
| `src/pool.js` | Single-active-model lock, auto-swap, idle timeout, pin-to-keep-loaded; resolves rkllm `max_context_len` at load (explicit option → per-model `ctx_window` → `DEFAULT_MAX_CONTEXT_LEN`=4096; the addon's own fallback is only 2048) and re-inits when it changes, so autoload/admin/chat all load with a consistent context; dispatches by extension at load time: `.gguf` → llama path (libPath = `LLAMA_RUNTIME_DIR/libllama.so`, auto-download if enabled), `.rkllm` → existing `runtimeCandidates()` path; `activeModel` includes `backend` field; `getStatus()` returns `activeRuntime: 'rkllm' | 'llama' | null` plus `loading: {model}|null` and `loadError: {model,message,code}|null`; `beginLoad()` starts a load without awaiting (the `/api/admin/load` route returns `202` immediately and the client polls `getStatus()` for `loading`/`loadError`/`isLoaded`, so a slow CPU-bound gguf load can't be reset by a reverse-proxy read timeout — `Models.vue`/`Bench.vue`/`Chat.vue` each have a `pollUntilLoaded`); the load timeout in `_tryLoadSlot` **scales with model file size** (~45s/GB, 60s floor, 15min cap) — a large GGUF offloaded to the GPU stages many GB into unified memory and legitimately takes minutes, so a flat 60s would fail it mid-upload (e.g. Gemma-4-26B staging ~16 GB to Vulkan); runtime version auto-discovery (`getAvailableRuntimes`, `readSoVersion`, `runtimeCandidates`, `_tryLoad`), caches winning lib path; `prefillAndCache` (abort-after-first-token KV warm); `generateSpeculative`/`generateEagle3`, `loadDraft`/`unloadDraft` for second worker slot. Pool size capped at chipset NPU core count (`getNpuCoreCount`); with >1 slot each model is pinned to its own core via `base_domain_id=(slot%cores)+1` (parallel models), single slot stays unpinned (all cores). Workers are forked with `workerEnv()` — `LD_LIBRARY_PATH` prepends the llama + rkllm runtime dirs so a dlopen'd `libllama.so` resolves its `libggml-*.so` siblings from the runtime dir even when a prebuilt bundle bakes an absolute CI build path into its RUNPATH (`LD_LIBRARY_PATH` is searched before `DT_RUNPATH`). `workerEnv({disableVulkan})` sets **`GGML_DISABLE_VULKAN=1`** unless the load requests TurboQuant KV (a `kv_type_k`/`kv_type_v` containing `turbo`): with the Vulkan backend present, llama.cpp would otherwise offload model **layers** to the Mali GPU, which splits work off the ork-NPU and corrupts the recurrent (Gated Delta Net) multi-turn path → 2nd-turn gibberish; keeping the GPU idle except for turbo holds layers on the NPU. **NPU execution precision + hybrid offload** (gguf path): `workerEnv` also sets ggml-ork's `ORK_QUANT` (NPU exec precision: `8`=W8A8 / `4`=W4A4 / unset = the runtime default, now pure INT4 with native multi-M) and `ORK_HYBRID=1` (re-enable the FFN/attn-only layer-wise hybrid loader; native-INT4 runtime defaults it off) — both read by the backend at init, so set at fork. `pool.js` resolves them from the per-model `npu_quant` (`auto`/`int4`/`int8`) and `npu_hybrid` settings: under `auto` a ≥5-bit GGUF (`ggufQuantBits` from the filename) raises to INT8 so a Q8/Q6/F16 file isn't silently downcast to 4-bit; `int4`/`int8` force it. For an **`.orkpack`** under `auto` the pack's own footer signature is the authority instead of the filename (`sigPrecision`) — the pack is the artifact, not the material, so an F16-named source packed at int8 must resolve to INT8; deriving `4` from the name would make `ork_sig_compatible` reject the pack, and a rejection reads as *stale* (rebuild, or `abort()` over `ORK_ORKPACK_MAX_REGEN_MB`). An explicit `int4`/`int8` still forces, and a real mismatch then fails loudly in `isOrkpackUsable` rather than silently regenerating a multi-GB pack. Carried on the load `options` as `ork_quant`/`ork_hybrid` and part of the slot-reuse key (a change forces a re-fork). Per-model settings are read through `modelSettingsFor(model)`, which falls back from a `.orkpack` id to the `.gguf` it was built from: a pack and its source are ONE logical model with TWO ids, and everything written against the gguf (the Quantization route’s `npu_quant`/`pack_mixed`, `speculative_mode`, `ctx_window`, sampling) was invisible once the pack became the servable entry, so a pack load silently took every default. The pack’s own row still wins when present. The whole per-model option block (KV types, `ork_quant`/`ork_hybrid`, `use_mmap`, `vk_mode`) is gated on `isLlamaBackedModel` — `.gguf` **or** `.orkpack`, since a pack is served by the same backend; gating on `.gguf` alone silently dropped every per-model setting for a pack-backed model. Both come from the single `resolveOrkPrecision(model, options, saved)`, exposed to the conversion scheduler as `pool.orkPrecision(model)` — a method rather than an import, because `pool.js` imports `conversion.js` and the reverse would cycle. They must be resolved identically for the pack BUILD and the SERVE: the pair is stamped into the `.orkpack` footer as its build signature, and the runtime refuses a pack whose signature its run does not match. `workerEnv` sets `ORK_NO_STUB=1`, and must **never** set `ORK_PERSIST` — it was removed upstream and now `GGML_ABORT`s the worker at backend init. **Automatic MCP background prefill**: when a model loads or settings/MCP servers change, `_generateMcpCaches` automatically runs in the background to sequentially pre-generate prefix caches for all enabled MCP tools and a combined catalogue (waiting for slots to be idle to never block live inference). |
| `src/conversion.js` | `.orkpack` conversion scheduler — builds the NPU-native pre-tiled weight cache for every servable `.gguf` so a load becomes a DMA copy instead of a dequant→quant→tile pass. Serial (single NPU stream), idle-driven (only when `pool.anyLoaded` is false), preemptible (a user Load kills the in-flight build and re-queues it). Spawns `llama-completion` rather than reusing the serving worker: the worker is hard-killed on unload (no clean ggml-ork teardown → no finalize), whereas a CLI exit finalizes. ggml-ork DERIVES the pack path from `-m` (`<model-dir>/<basename>.orkpack` — exactly `orkpackPathFor`), so no path env is needed; the spawn sets `ORK_NO_STUB=1` plus the model's precision env, with args `--device ORK -ngl 99 --no-repack -p x -n 1` (a single 1-token forward packs every weight — ggml-ork exempts WRITE mode from its `ORK_MINM` and MoE M=1 declines precisely so this works; `--device ORK` pins the matmuls to the NPU so a bare `-ngl` can't hand the layers to Vulkan and pack zero weights). `pool.js`'s cold-start guard calls `convertNow(rel, envExtra)` to build-then-load so a GGUF never serves cold; `<pack>.tmp` growth vs source size drives the UI progress sidecar `<pack>.json`. `removeOrkpack` sweeps the pack plus every sidecar that belongs to it (`.tmp`/`.json`/`.meta.json`/`.prov.json`/`.gmax`/`.gguf`) — ggml-ork only unlinks `.gmax` when IT sees the pack as stale, so deleting the pack first would otherwise attach an old tuning profile to a freshly built one. Skips architectures the NPU MUL_MAT accelerator can't pack (`qwen35` SSM/GDN-hybrid; `dflash` draft heads, which run co-resident and have no pack of their own) |
| `src/orkpack.js` | Minimal `.orkpack` footer reader — the authoritative freshness check. The 32-byte footer at EOF (`index_off`/`n_entries`/`version`/`ork_fmt`/`quant_sig`/`magic`) is self-describing, and ggml-ork validates exactly those fields before adopting a pack; whatever it rejects is *stale*, and a stale pack over `ORK_ORKPACK_MAX_REGEN_MB` (default 2048) makes the runtime **`abort()`** rather than rebuild. `isOrkpackUsable(path, precision)` mirrors that gate: magic `ORKPK01`, `version === 6` **exactly** (v7/v8 were live briefly upstream then collapsed back into 6, so those numbers now denote a different layout wearing a reused number), `index_off < size`, the learned `ork_fmt`, and `sigCompatible` — plus `sigPrecision(sig)`, the inverse: the `{orkQuant, orkHybrid}` a run must adopt to be compatible with a given pack, which the load path uses as the authority under `npu_quant: auto` — where `ORK_HYBRID` is **prescriptive** (it changes which tensors are packed at all, so it must match) and `ORK_QUANT` is **descriptive** (a run forcing no precision simply adopts the pack's tier). **Provenance sidecar** (`<pack>.prov.json`, `writeOrkpackProvenance`/`readOrkpackProvenance`/`provenanceChipsetMismatch`): a `.orkpack` is a hardware-specific build artifact — weights pre-tiled for one NPU's MAC geometry by one ork-driver against one kernel driver — yet its footer records only what the runtime needs in order to decide adoption and says nothing about the machine that built it, so a pack that turns up on a box (copied, restored, served off a shared `MODELS_DIR`) is unattributable. This is the problem a `pg_dump` header solves, and the answer is the same: stamp the build environment beside the artifact at generation time. `conversion.js` writes it in `_spawnBuild`'s `done()` — chipset + `chipsetSource` + NPU core count + NPU kernel-driver version, OS pretty name + kernel release + hostname, the llama-runtime build tag, the **ork-driver version the build itself printed** (scraped from the child's `[ork] ork-driver 1.0.99 (W4A4)` line — only the runtime knows it), the precision/CLI configuration, the footer as written, and the source GGUF's path/size/mtime. Deliberately **advisory, never a gate**: freshness stays the footer's job, because a pack built by a bare `llama-completion` run outside oRKLLM has no sidecar at all and treating absent as stale would condemn a good pack (and abort the runtime over `ORK_ORKPACK_MAX_REGEN_MB`). `provenanceChipsetMismatch` is one-directional for the same reason — only a chipset that is *present in the record* and differs from the detected one is a mismatch; no record, no recorded chipset, or an undetectable local chipset all return null (unknown). `pool.js` logs that mismatch on a pack load and folds it into the `ORKPACK_UNUSABLE` error (as `ORKPACK_WRONG_CHIPSET`), since another chip's tiling is the likeliest explanation for a rejection the footer alone cannot express. The real fix belongs upstream — a provenance block *inside* the pack cannot be separated from it by a file copy, and a sidecar can. Deliberately re-implemented in JS rather than calling the exported `ggml_backend_ork_orkpack_valid`: that symbol lives in `libggml-ork.so`, which the WORKER loads while the question is asked in the MAIN process, and it reads `ORK_QUANT`/`ORK_HYBRID` from the *calling* process's environment so it cannot answer per-model — parsing the few bytes that matter is the same trick `gguf.js` uses for GGUF headers. `ork_fmt` (ork-driver's on-disk format token) cannot be computed without loading the runtime, so it is **learned**: `recordOrkFmt` stamps it from the footer of the first pack the installed runtime builds, keyed by build tag **and chipset** (`getPlatform()`) in `LLAMA_RUNTIME_DIR/orkpack-fmt.json`. The chipset half of the key is **forward-looking, not a live concern**: the initial release deliberately targets a single chipset (RK3588), so there is no second chip for a pack to arrive from. It matters later because a `.orkpack` is inherently chipset-specific — it holds weights pre-tiled for that NPU's MAC geometry, and every Rockchip NPU generation implements it differently, so packs will have to be rebuilt per chip rather than shared. Nothing in the footer we read states which chip built a pack, so once multi-chipset support lands a cross-chipset pack is only rejected if ork-driver's own `ork_fmt` token happens to differ per chip (unverified) — the platform key at least stops a shared or copied state file (NFS `MODELS_DIR`, a copied models tree) from certifying another chip's format as this one's. Unknown platform degrades to tag-only, and while `ork_fmt` is unknown it is simply not used as a signal, so both fail closed. This replaced a sidecar comparing the llama-runtime build **tag**, which was wrong in both directions — too coarse (the fork cuts ~8 build tags a day, so every runtime sync invalidated and rebuilt every pack on the box, though ork-driver's format token has not moved since 2026-07-20) and too loose (a tag match said nothing about the precision signature, so a pack built at one precision looked fresh to a load that would serve at another) |
| `src/config.js` (chipset) | `getPlatform()` (SoC slug **read from the kernel**, never configured — oRKLLM only ever runs on Rockchip silicon, so the SoC is a property of the machine the OS already knows. Sources are probed in order of authority: `/proc/device-tree/compatible` → `/sys/firmware/devicetree/base/compatible` (the same data, but `/proc/device-tree` is only conventionally a symlink to it) → `/sys/devices/soc0/{machine,family,soc_id}` → `/proc/cpuinfo`. `parseSocSlug(raw)` is the pure, unit-tested half — prefers a `rockchip,rkNNNN` tuple, falls back to a bare `rkNNNN` token for the nodes that carry only a bare name; variant suffixes are kept verbatim (`rk3588s` ≠ `rk3588`) so each consumer decides what to treat as equivalent. `getPlatformSource()` names the node that answered, for diagnostics. A null on ARM64 Linux is an anomaly, not a normal state, so it is logged once) + `getNpuCoreCount()` (rk3576→2, rk3588→3, else 1) + `getGpuInfo()` (Mali model + shader-core count parsed from the kernel `gpuinfo` node, e.g. "Mali-G610 4 cores" → `{model:'Mali-G610',cores:4}`) + `getDeviceDrivers()` (kernel driver in use + version per device: **NPU** from `/sys/kernel/debug/rknpu/version` → `{name:'RKNPU',version:'0.9.8'}`; **GPU** = the driver bound to the Mali platform device (`mali`/`panfrost` via the `…/driver` symlink) + the Mali kernel DDK version best-effort from `dmesg` "Kernel DDK version g25p0-…" → `{name:'mali',version:'g25p0'}`; cached, null off-board) — single source of truth, surfaced in `/api/admin/status` as `platform`/`npuCores`/`gpu`/`drivers` (and `/api/admin/global-settings` `server` carries `platform`/`npuCores`; the Settings NPU-pool-size slider caps its max at `npuCores`; the Dashboard Compute table shows the GPU model + shader-core count from `status.gpu` and a **Driver** column — driver name + version — from `status.drivers`) |
...
| `frontend/src/views/Models.vue` | Three tabs — **Manager**, **Downloader**, **Quantization**. The Quantization tab builds a `.gguf` into its `.orkpack` at a chosen tier: pick a source (each row chipped with its weight width and flagged uncompressed/already-quantized, since 4-bit from an already-quantized source compounds error), pick int4 or int8, and optionally Mixed precision (int4 only — an int8 pack has nothing to promote to) with a promotion budget. `POST /api/admin/quantize` returns 202 and the tab polls `/api/admin/library` `orkConversion` `{status,progress,label}` — the same state the Manager shows for an idle build, so there is no second job registry. Needs a llama runtime >= `b10701-ork` (before that, qerr was recorded only under `ORK_GPTQ` so nothing could be ranked, and `--pack-bits` set the pack signature but not its tier — llama.cpp-rockchip#2 and #4). A mixed build is two passes and runs them itself (`label` reads "pass 1 of 2 - measuring" / "pass 2 of 2 - promoting"): promotion is ranked by qerr, the per-weight quantisation error only a completed pack records, so pass 1 must build uniform before pass 2 can read it back via `--pack-qerr-source`. The route also persists the chosen tier to the model’s `npu_quant`/`pack_mixed` settings — mandatory, because the pack’s precision is stamped in its footer and a serving run that disagrees has the pack refused as stale. Model manager + HF search/collection browser/downloader; recursive model scan for `.rkllm` and `.gguf`; platform-aware search; download queue grouped by repo; runtime chip on each model (`rkllm / .rkllm` or `llama / .gguf`). Manager tab uses `GET /api/admin/library` to show three categories — Available Models (`.rkllm` and `.gguf`, each with `thinkingToggle`), Base Models, and Eagle-3 Draft Heads (with an "Add embeddings" dialog: downloaded base model or base-repo slice → `POST /api/admin/eagle3/embeddings`). The per-model settings dialog shows the **Enable Thinking** switch but disables it (forced on) when `thinkingToggle === false` (a model with no non-thinking mode, e.g. LFM2.5-MoE), with a caption explaining the model always reasons; `openSettings` pulls `thinkingToggle` from the library entry since the manager list comes from `/v1/models` (which omits it). The "KV Cache Compression" dropdown (`kvCacheItems` computed) is **backend-specific**: for `runtime === 'llama'` it sets the in-context KV V-cache type (Off/FP16, q8_0, TurboQuant turbo2/3/4 — live, GPU); for rkllm it selects the SSD-blob PolarQuant scheme (Off, q8, pq8, pq4). `openSettings` restores `kv_cache_quant` from saved settings (previously omitted → reset on open). Wide-screen responsive (`.page-container` 1400px): main-list + sidebar on lg+. Downloader: a repo with multiple weight files (e.g. several GGUF quants/shards) shows a **file picker** with a per-file quant chip (`quantOf()` parses Q4_K_M/Q5_K_M/Q6_K/Q8_0/IQ…/F16) instead of auto-downloading every variant; single-weight repos still auto-download. `_readJson()` gives a clean "server restarting" message for non-JSON (nginx 502) responses instead of "Unexpected token '<'" |
