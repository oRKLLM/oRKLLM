...
| `src/spv_sync.js` | Opt-in Vulkan SPIR-V shader fetch (Eagle-3 `vulkan` draft) — same model as runtime_sync: downloads the latest `ggml-vulkan-spirv-<tag>.tar.gz` from `SPV_MIRRORS` (default `oRKLLM/llama.cpp`, override `ORKLLM_SPV_MIRRORS`), verifies its `.sha256`, extracts the `.spv` modules + `manifest.json` into `SPV_DIR` (`ORKLLM_SPV_DIR`, exposed to the addon's Vulkan loader via env), records the tag; ARM64-Linux only. `isSpvAvailable()` gates the Eagle-3 `vulkan` option. `auto_download_spv` setting; `GET /api/admin/spv`, `POST /api/admin/spv/sync`; `spvAvailable` in `/api/admin/status` |
