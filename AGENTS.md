...
| `frontend/src/views/Models.vue` | Model manager + HF search/collection browser/downloader; recursive model scan for `.rkllm` and `.gguf`; platform-aware search; download queue grouped by repo; runtime chip on each model (`rkllm / .rkllm` or `llama / .gguf`). Manager tab uses `GET /api/admin/library` to show three categories — Available Models (`.rkllm` and `.gguf`, each with `thinkingToggle`), Base Models, and Eagle-3 Draft Heads (with an "Add embeddings" dialog: downloaded base model or base-repo slice → `POST /api/admin/eagle3/embeddings`). The per-model settings dialog hides the **Enable Thinking** switch when `thinkingToggle === false` (a model with no non-thinking mode, e.g. LFM2.5-MoE). Wide-screen responsive (`.page-container` 1400px): main-list + sidebar on lg+. Downloader: a repo with multiple weight files (e.g. several GGUF quants/shards) shows a **file picker** with a per-file quant chip (`quantOf()` parses Q4_K_M/Q5_K_M/Q6_K/Q8_0/IQ…/F16) instead of auto-downloading every variant; single-weight repos still auto-download. `_readJson()` gives a clean "server restarting" message for non-JSON (nginx 502) responses instead of "Unexpected token '<'" |
