...
* **Prefix KV Cache**: Tiered SSD hot/cold LRU cache saves KV state between conversation turns. Newly written or promoted hot cache entries are automatically mirrored to the cold cache SSD storage if space is available. Sliding context window (configurable up to 32,768 tokens, default 8,192) prevents NPU OOM on long conversations.
